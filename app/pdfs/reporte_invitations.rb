...
class ReporteInvitations < Prawn::Document
  ...
  def header
    ...
    move_up 10
    image "#{Rails.root}/app/assets/images/cebolla.png", width: 70, height: 45, :position => 70
    move_up 30
    image "#{Rails.root}/app/assets/images/logo_ulab.png", width: 80, height: 60, :position => 430
    font("#{Prawn::DATADIR}/fonts/DejaVuSans.ttf") do 
    	draw_text "UNIVERSIDAD SIMÓN BOLÍVAR", :at =>[22,667]
      font_size 10
    	draw_text "Vicerectorado Académico", :at => [43,655]
      font_size 8
      draw_text "Unidad de laboratorios", :at =>  [58, 647]
    end
 ...
 def tittle 
  ...
  def encabezado
 	 draw_text "Sartenejas, #{Time.now.strftime("%Y-%d-%m")}", :at => [415, 590]
  end
  ...
  def texto
    ...
    text_box "• Especificaciones Técnicas", :at => [0, 445]
    text_box "• Monto de la Oferta", :at => [0, 430]
    text_box "• Modalidad de pago", :at => [0, 415]
    text_box "• Garantía", :at => [0, 400]     
	 text_box "• Tiempo de entrega",:at => [160, 445]
    text_box "• Validez de la oferta", :at => [160, 430]
    text_box "• Domicilio Principal ",:at => [160, 415]
    text_box " (Dirección de la Empresa)",:at => [168,400]
    text_box "• Constancia de Inscripción en el Registro" , :at => [317, 445]
    text_box "  de Proveedores Interno USB.", :at => [323,430]
    text_box "• Valor Agregado Nacional (Art. 9 de " ,:at => [315, 415]
    text_box "la Ley de Contrataciones Públicas).", :at => [323,400]
    stroke do
    	vertical_line 445, 390, :at => 153
      vertical_line 445, 390, :at => 312
      
    end

    move_down 80
    text " La oferta deberá ser enviada por fax al número: #{@invitation.telefono} o correo electrónico #{@invitation.correo} sellado y 				firmado por la empresa participante. De igual manera, le informamos que se recibirán ofertas hasta el día #{time.strftime("%Y-%d-%m")}", :align => :justify, :indent_paragraphs => 40
   ...
   stroke do
   	horizontal_line 210, 340, :at => 75
		horizontal_line 375, 505, :at => 35
   end

   draw_text "#{@invitation.responsable}", :at => [235,60]
   draw_text "Recibido por", :at => [410,60], :size => 8
